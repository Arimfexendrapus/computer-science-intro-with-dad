...
//if the user wants to play a game the program continues
    ...
    // asks whether the user wants to play hangman
    {
         ...
         //input validation
         {
            while (ans.compare("yes")!=0 && ans.compare("no")!=0 && ans.compare("YES")!=0 && ans.compare("NO")!=0)
            {
                cout<<"TRY AGAIN"<<endl<<"HOW ABOUT A NICE FRIENDLY GAME OF HANGMAN?"<< endl<<"YES / NO"<<endl;
                cin>>ans;
            }
...
string   incorrectletters,ans, blanks,cword="computer game";
char letter;
int incorrect=0;


//instructions
cout<< "THE GOAL IS TO GUESS THE RIGHT WORD OR PHRASE, ONE LETTER AT A TIME; YOU HAVE 10 INCORRECT GUESSES BEFORE YOU LOSE."<<endl;
...
srand(time(0));
cword=words[rand() % ii+1];

//obtaining the word length and outputting blanks
int word_length;
word_length=cword.size();
cout<< "THE WORD OR PHRASE IS "<<word_length<<" LETTERS LONG, INCLUDING SPACES."<<endl;
